...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_namespace WHERE nspname = 'postgis') THEN
    RAISE NOTICE 'Creating schema postgis for Customer Service...';
    CREATE SCHEMA postgis;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'postgis') THEN
    RAISE NOTICE 'Creating extension postgis for Customer Service...';
    CREATE EXTENSION postgis SCHEMA postgis;
  ELSE
    RAISE NOTICE 'PostGIS extension already exists for Customer Service.';
  END IF;
END
$$;
...
BEGIN
  RAISE NOTICE 'Processing tables for geom column in Customer Service...';
  FOR tbl_name IN
    SELECT t.table_name
    FROM information_schema.tables t
    WHERE t.table_schema = schema_name
      AND t.table_name IN ('Vendor', 'VendorLocation') -- Customize for Vendor Service tables
  LOOP
    IF EXISTS (
      SELECT 1
      ...
      RAISE NOTICE 'Adding geom column to table % in Vendor Service...', tbl_name;
      ...
      RAISE NOTICE 'Creating spatial index for table % in Vendor Service...', tbl_name;
      EXECUTE format('CREATE INDEX %I ON %I USING GIST (geom)', tbl_name || '_geom_idx', tbl_name);
    ELSE
      RAISE NOTICE 'Table % does not meet criteria for geom column (missing longitude/latitude or geom exists).', tbl_name;
    END IF;
  END LOOP;
  RAISE NOTICE 'Geom column processing completed for Vendor Service.';
END
$$;
